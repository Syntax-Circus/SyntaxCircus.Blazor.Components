...
No service registration, JavaScript, CSS import, or middleware registration is required for the
components above. `IFileDownloadService` (below) is the one exception: it is an opt-in JavaScript
interop service, not a component, and does require registration.
...
|---|---|---|
| `IFileDownloadService` | `SyntaxCircus.Blazor.Components.Interop` | Saves generated bytes or a stream as a file in the browser. Requires registration via `AddFileDownload()`. |
...
`IFileDownloadService` saves generated bytes or a stream as a file in the browser: it turns the
content into a client-side blob and triggers a temporary anchor download. It is the one service
this package registers, and it is the one JavaScript module besides `ReconnectModal`'s — both are
documented, intentional exceptions to this package otherwise shipping no JavaScript and no service
registration.

It works only in an **interactive** Blazor render mode (Server or WebAssembly), because it needs a
JavaScript runtime to invoke. Calling it from a statically server-rendered (SSR) component has no
effect — there is no client-side script host to run the download in.
...
This registers a scoped `IFileDownloadService`/`FileDownloadService`.

### Use it from a component or another service

```razor
@inject IFileDownloadService FileDownload

<button type="button" @onclick="DownloadReportAsync">Download report</button>

@code {
    private async Task DownloadReportAsync()
    {
        byte[] pdfBytes = await GenerateReportAsync();
        await FileDownload.DownloadAsync("report.pdf", "application/pdf", pdfBytes);
    }
}
```

A `Stream` overload is also available for content that is already being read from a stream; the
stream is read to its end but not disposed by the service, so the caller remains responsible for
disposing it. An empty or whitespace `contentType` falls back to `application/octet-stream`.

```csharp
await FileDownload.DownloadAsync("export.json", "application/json", exportStream);
```

`FileDownloadService` implements `IAsyncDisposable` and lazily imports its JavaScript module on
first use; disposing it (for example, as part of DI scope disposal at the end of a Blazor Server
circuit) disposes that module reference and swallows `JSDisconnectedException`, since the circuit
can already be gone by the time disposal runs.
...
- register services or middleware, other than the opt-in `AddFileDownload()` behind `IFileDownloadService`;
