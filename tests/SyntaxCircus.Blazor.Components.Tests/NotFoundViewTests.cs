...
public sealed class NotFoundViewTests
{
    ...
    public void Render_WithContentSlots_RendersSlotsInsteadOfDefaultBodyAndActions()
    {
        ...
        RenderFragment header = builder => builder.AddContent(0, "Custom heading");
        ...
        markup.ShouldNotContain(">Missing record</h1>");
    ...
    [Fact]
    public void Render_WithoutActionContent_CanSuppressDefaultAction()
    {
        using var context = new BunitContext();
        var markup = context.Render<NotFoundView>(parameters => parameters
            .Add(component => component.ShowDefaultActions, false))
            .Markup;

        markup.ShouldNotContain("data-not-found-actions");
        markup.ShouldNotContain(">Go home</a>");
    }
}
